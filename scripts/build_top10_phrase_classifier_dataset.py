...
OUTPUT_PATH = Path(r"C:\Users\Joe\OneDrive\Desktop\signmatic_thesis\experiments\top10_phrases_classifier\data")
...
# -------------------------
# Step 1: get common phrases across train/val/test
# -------------------------
train_df = load_labels_df("train")
val_df = load_labels_df("val")
test_df = load_labels_df("test")

train_counts = Counter(train_df["clean_sentence"])
val_counts = Counter(val_df["clean_sentence"])
test_counts = Counter(test_df["clean_sentence"])

candidate_phrases = []
for phrase, train_count in train_counts.most_common():
    if (
        train_count >= MIN_COUNT_TRAIN
        and val_counts[phrase] >= MIN_COUNT_VAL
        and test_counts[phrase] >= MIN_COUNT_TEST
    ):
        candidate_phrases.append(phrase)

top_phrases = candidate_phrases[:TOP_K]
top_phrase_set = set(top_phrases)

print(f"Selected top {len(top_phrases)} phrases present in all splits:")
for i, phrase in enumerate(top_phrases, start=1):
    print(
        f"{i}. {phrase} "
        f"(train={train_counts[phrase]}, val={val_counts[phrase]}, test={test_counts[phrase]})"
    )
...
def build_split(split, max_per_class):
    ...
    np.save(OUTPUT_PATH / f"X_{split}_top10.npy", X)
    np.save(OUTPUT_PATH / f"y_{split}_top10.npy", y)
    save_lines(OUTPUT_PATH / f"y_text_{split}_top10.txt", y_text)
    save_lines(OUTPUT_PATH / f"clip_names_{split}_top10.txt", clip_names)
    ...
    print("Class counts:")
    for phrase, cnt in split_counts.items():
        print(f"{phrase}: {cnt}")
