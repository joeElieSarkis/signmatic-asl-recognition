...
# PATHS
# =========================
SOURCE_PATH = Path(r"C:\Users\Joe\OneDrive\Desktop\signmatic_thesis\experiments\face_10words\data")
OUTPUT_PATH = Path(r"C:\Users\Joe\OneDrive\Desktop\signmatic_thesis\experiments\face_10words_normalized\data")
...
def normalize_sequence(sequence):
    ...
    for i in range(sequence.shape[0]):
        out[i] = normalize_frame(sequence[i])
    return out.astype(np.float32)


def process_split(split):
    x_path = SOURCE_PATH / f"X_{split}_face_10w.npy"
    y_path = SOURCE_PATH / f"y_{split}_face_10w.txt"
    clip_path = SOURCE_PATH / f"clip_names_{split}_face_10w.txt"

    X = np.load(x_path)
    y = load_lines(y_path)
    clip_names = load_lines(clip_path)
    ...
    for i in range(len(X)):
        ...
        if (i + 1) % 100 == 0:
            print(f"Normalized {i+1}/{len(X)}")

    np.save(OUTPUT_PATH / f"X_{split}_face_10w_norm.npy", X_norm)
    save_lines(OUTPUT_PATH / f"y_{split}_face_10w_norm.txt", y)
    save_lines(OUTPUT_PATH / f"clip_names_{split}_face_10w_norm.txt", clip_names)

    print(f"{split} done.")
    print("Saved shape:", X_norm.shape)
    print("Example first frame values:", X_norm[0, 0, :12])
...
if __name__ == "__main__":
    ...
    src_vocab = SOURCE_PATH / "vocab_face_10w.json"
    dst_vocab = OUTPUT_PATH / "vocab_face_10w_norm.json"
