...
MODEL_PATH = Path(r"C:\Users\Joe\OneDrive\Desktop\signmatic_thesis\experiments\face_4words_balanced\models\best_face_4w_balanced_seq2seq_transformer.pt")
...
def greedy_decode(model, src, max_tokens, sos_idx, eos_idx, pad_idx):
    """
    Generate sentence token by token.
    """
    generated = [sos_idx]

    for _ in range(max_tokens - 1):
        tgt_input = torch.tensor([generated], dtype=torch.long, device=src.device)
        outputs = model(src, tgt_input)  # (1, tgt_len, vocab_size)
...
results_file = OUTPUT_PATH / "evaluation_results_face_4w_balanced_seq2seq.txt"
