...
MODEL_OUTPUT = Path(r"C:\Users\Joe\OneDrive\Desktop\signmatic_thesis\experiments\full_face_seq2seq\models")
...
class SignDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x = torch.tensor(self.X[idx], dtype=torch.float32).clone()

        # Scale x and y coordinates for numerical stability
        # x coordinates are at positions 0, 3, 6, ...
        # y coordinates are at positions 1, 4, 7, ...
        # confidence values stay unchanged
        x[:, 0::3] = x[:, 0::3] / 1000.0
        x[:, 1::3] = x[:, 1::3] / 1000.0
...
# Load vocabulary
# =========================
with open(BASE_PATH / "vocab_face_full.json", "r", encoding="utf-8") as f:
    vocab = json.load(f)
...
train_dataset = SignDataset(
    BASE_PATH / "X_train_face_full.npy",
    BASE_PATH / "y_train_face_full.txt",
    vocab
)

val_dataset = SignDataset(
    BASE_PATH / "X_val_face_full.npy",
    BASE_PATH / "y_val_face_full.txt",
    vocab
)
...
for epoch in range(EPOCHS):
    ...
    if val_loss < best_val_loss:
        ...
        epochs_without_improvement = 0
        torch.save(model.state_dict(), MODEL_OUTPUT / "best_full_face_seq2seq_transformer.pt")
